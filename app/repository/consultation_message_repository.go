...
func (repo *ConsultationMessageRepositoryImpl) Create(ctx context.Context, message entity.ConsultationMessage) (*entity.ConsultationMessage, error) {
	const create = `
	INSERT INTO consultation_messages (session_id, sender_id, message, attachment)
	VALUES ($1, $2, $3, $4)
	RETURNING id, session_id, sender_id, message, attachment, created_at, updated_at`

	row := repo.db.QueryRowContext(ctx, create, message.SessionId, message.SenderId, message.Message, message.Attachment)
	var created entity.ConsultationMessage
	err := row.Scan(&created.Id, &created.SessionId, &created.SenderId, &created.Message, &created.Attachment, &created.CreatedAt, &created.UpdatedAt)
